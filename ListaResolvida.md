...
if (idade < 16) {
    ...
    var podeVotar = true;
    var votoObrigatorio = false;
} else if (idade >= 18) {
    var podeVotar = true;
    var votoObrigatorio = true;
}
```
______
...
Class Circulo herda FormaGeometrica:
    Atributos:
        - cor (vem da classe)
    Método CalcularArea (Circulo):
        let area = pi * raio^2
        console.log(area)

Class Retangulo herda FormaGeometrica:
    Atributos: 
        - cor (vem da classe)
    Método CalcularArea (Retangulo):
        let area2 = lado
```
______
...
SimulaçãoDeCorrida
    function SimularCorrida (v0, a, d, V, t);
    v -> v0
    t -> 0

    Durante os Testes:
        v -> v0 + a * t (Mudar a velocidade de acordo com a aceleração e tempo)

        Se (v > V){ (Verifica se a velocidade máxima foi burlada)
        v -> V
        }

        dP -> v * t (Calcular a distância percorrida até o momento)

        Se (dP >= d) { (observar se a distância foi toda percorrida)
